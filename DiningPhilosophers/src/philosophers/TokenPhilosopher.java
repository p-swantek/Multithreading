...
/**
 * Implementation of a Philosopher that will need to obtain a token in 
 * order to access the table to get the forks and start eating.  Deadlock
 * is avoided because not all Philosophers will be able to access the table at once
 * due to the number of available tokens
 * 
...
class TokenPhilosopherImpl implements TokenPhilosopher, Runnable{
    ...
    /**
     * This philosopher will try to aquire a token to access the table, waiting until it does.
     * When it gets access to the table, it will pick up the forks, eat/think, then stop. If interrupted
     * or done eating, this philosopher will relinquish the token and leave the table
     */
    @Override
    public void run (){
		...
		try {
			while (true){
				//Added some new debug outputs around when the philosopher calls aquire() and release() methods
				Debug.out.println (name + " is trying to aquire a permit");
				token.acquire(); //try to aquire a spot at the table, will wait if no token is currently available

 ...
 * A philosopher thinks, then chooses which fork to pick
 * up based on which fork has a smaller number so that the forks are picked 
 * up in order.  A philosopher will always try to pick up the fork with the lower
 * number first.
 * This version of the business logic has been edited by Peter Swantek.
 * All of my edits are highlighted with EDITED by Peter Swantek.
...
public interface OrderedPhilosopher extends Philosopher{
     ...
     */
	// EDITED by Peter Swantek:
	// changed the factory to be the one that will produce the OrderedPhilosopher
...
class OrderedPhilosopherFactoryImpl implements PhilosopherFactory {
	...
	// EDITED by Peter Swantek:
	// changed the build method so that it builds an instance of an OrderedPhilosopher
	@Override
...
class OrderedPhilosopherImpl implements Runnable, OrderedPhilosopher {
	...
	// EDITED by Peter Swantek:
	// changed the locks to be Comparables rather than Objects in order to use the compareTo() method to assign order to the forks
    ...
    // EDITED by Peter Swantek:
    // changed the constructor to take Comparables to represent the forks as opposed to Objects
    public OrderedPhilosopherImpl(final Comparable lhFork, final Comparable rhFork, final String name){
	    ...
	    // if the lhFork that is passed into the constructor is greater than the rhFork based on the compareTo() result,
	    // swap the values so the lhFork instance variable always holds a reference to the lower number and the rhFork always holds the larger number

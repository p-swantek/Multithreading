...
public interface OptimisticMutableList extends MutableList {
    ...
    public static final MutableListFactory factory = new OptimisticMutableListFactoryImpl(); // factory
                                                                                             // to
                                                                                             // build
                                                                                             // OptimisticMutableLists
}
...
class OptimisticMutableListFactoryImpl implements MutableListFactory {
    ...
    public MutableList build() {
        return new OptimisticMutableListImpl(); // Have the
                                                // OptimisticMutableList factory
                                                // build an instance of the
                                                // OptimisticMutableListImpl
    }
}
...
class OptimisticMutableListImpl implements OptimisticMutableList {
    ...
    private final AtomicReference<ImmutableList> immutableRef; // reference to
                                                               // the
                                                               // ImmutableList

    public OptimisticMutableListImpl() {
        immutableRef = new AtomicReference<ImmutableList>(ImmutableList.empty); // construct
                                                                                // an
                                                                                // atomic
                                                                                // reference
                                                                                // to
    ...
    public void add(Object element) {
        ...
        do {
            ...
            newList = oldList.cons(element); // create a new list that contains
                                             // the item that was added
        } while (!immutableRef.compareAndSet(oldList, newList)); // keep looping
                                                                 // until
                                                                 // compareAndSet
                                                                 // succeeds
    }
    ...
    public void remove(Object element) {
        ...
        do {
            oldList = immutableRef.get(); // get the old value
            newList = oldList.remove(element); // create a new list that has the

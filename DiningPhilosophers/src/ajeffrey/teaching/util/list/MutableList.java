...
/**
 * An ordered collection of elements. This is a cut down version of the Java collections library, intended for teaching purposes.
 * 
 * @author Alan Jeffrey
 * @version 1.0.4
...
public interface MutableList {
     ...
     * @exception NoSuchElementException thrown if the element is not in the list.
     */
...
class MutableListImpl implements MutableList {
    ...
    public void add(final Object element) {
        synchronized (lock) {
            Debug.out.println("MutableListImpl.add: Starting...");
            final ImmutableList oldContents = contents;
            Debug.out.println("MutableListImpl.add: Calling " + oldContents + ".cons (" + element + ")");
            final ImmutableList newContents = oldContents.cons(element);
            Debug.out.println("MutableListImpl.add: setting contents = " + newContents);
            contents = newContents;
            Debug.out.println("MutableListImpl.add: ...done.");
        }
    }

    public void remove(final Object element) {
        synchronized (lock) {
            Debug.out.println("MutableListImpl.remove: Starting...");
            final ImmutableList oldContents = contents;
            Debug.out.println("MutableListImpl.remove: Calling " + oldContents + ".remove (" + element + ")");
            final ImmutableList newContents = oldContents.remove(element);
            Debug.out.println("MutableListImpl.remove: setting contents = " + newContents);
            contents = newContents;
            Debug.out.println("MutableListImpl.remove: ...done.");
        }
    }

...
public interface SocketIO {
     ...
     */
    public PrintStream buildPrintStream(Socket s) throws IOException;
     ...
     */
    public Socket buildSocket(URL url, int defaultPort) throws IOException, UnknownHostException;
...
class SocketIOImpl implements SocketIO {
    ...
    public InputStream buildInputStream(final Socket s) throws IOException {
        return new SocketInputStream(s);
    }

    public OutputStream buildOutputStream(final Socket s) throws IOException {
        return new SocketOutputStream(s);
    }

    public Reader buildReader(final Socket s) throws IOException {
        return new InputStreamReader(buildInputStream(s));
    }

    public Writer buildWriter(final Socket s) throws IOException {
        return new OutputStreamWriter(buildOutputStream(s));
    }

    public BufferedReader buildBufferedReader(final Socket s) throws IOException {
        return new BufferedReader(buildReader(s));
    }

    public SafeBufferedReader buildSafeBufferedReader(final Socket s) throws IOException {
        return new SafeBufferedReader(buildReader(s));
    }

    public PrintWriter buildPrintWriter(final Socket s) throws IOException {
        return new PrintWriter(buildWriter(s), true);
    }

    public SafePrintWriter buildSafePrintWriter(final Socket s, final String lineSep) throws IOException {
        return new SafePrintWriter(buildWriter(s), true, lineSep);
    }

    public BufferedInputStream buildBufferedInputStream(final Socket s) throws IOException {
        return new BufferedInputStream(buildInputStream(s));
    }

    public PrintStream buildPrintStream(final Socket s) throws IOException {
        return new PrintStream(buildOutputStream(s), true);
    }

    public Socket buildSocket(final URL url, final int defaultPort) throws IOException, UnknownHostException {
        if (url.getPort() == -1) {
...
class SocketOutputStream extends FilterOutputStream {
    ...
    protected SocketOutputStream(final Socket socket) throws IOException {
...
class SocketInputStream extends FilterInputStream {
    ...
    protected SocketInputStream(final Socket socket) throws IOException {
